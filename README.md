...
## 🎯 三大检视维度与前置拦截规程
- **Step 1.0 (需求背景拦截)**：检视前校验需求背景与旨在解决的核心问题，缺失则主动拦截。
- **维度 1: 体验目标的达成 (旅程走查)**：校验体验目标与剧本故事（若已有则直接推进不拦截，缺失则触发前置拦截）。
- **维度 2: Checklist 的检视**: 根据界面功能特征自动匹配公共与特定场景子维度，深层逐条核查。
- **维度 3: 基于体验愿景的方案提质**: 从专家顶层视角寻找能提升产品高级感与品牌拉力的优化点。
- **报告生成**: 生成包含截图批注、严格 P1-P4 定级（最下方附带业务后果判定表）与 To-Be 改进方案的专业报告。
